...
def preprocess_examples(examples):
    query = [SPECIAL_TOKENS_MAP["human"] + str(examples["query"])]    
    persona = [SPECIAL_TOKENS_MAP["persona"]] + ast.literal_eval(examples['ground_persona'])
    knowledge = [SPECIAL_TOKENS_MAP["knowledge"]] + [str(examples["ground_knowledge"])]
    answer = examples['answer'] 
    text = persona + knowledge + query
...
if __name__ == "__main__":
    ...
    df = pd.read_csv("/work/kanakr/chat_persona/data/focus_val_data.csv")
    row = df.iloc[5]
